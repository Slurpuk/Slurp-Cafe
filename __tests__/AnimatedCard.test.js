    ...
    describe('Displays correctly', function () {
        it('Displays Delete button when passed in bottom fixed', async function () {
            const {getByTestId} = render(<AnimatedCard
                collapsableContent={<OrderCardContext.Provider value={orderCardContextMock}>
                    <AnimatedCardContext.Provider value={animatedCardContextMock}>
                        <ReducedOrder toFixed/>
                    </AnimatedCardContext.Provider>
                </OrderCardContext.Provider>}
                hidableContent={<OrderCardContext.Provider value={orderCardContextMock}>
                    <AnimatedCardContext.Provider value={animatedCardContextMock}>
                        <ExpandedOrder toFixed/>
                    </AnimatedCardContext.Provider>
                </OrderCardContext.Provider>}
                initialHeight={Dimensions.get('window').height * 0.14}
                bottomFixed={<OrderCardContext.Provider value={orderCardContextMock}>
                    <DeleteButton/>
                </OrderCardContext.Provider>}
            />);
            ...
            // const pressableDelete=getByTestId('pressableDelete');
            // fireEvent(pressableDelete, 'press');

        });
        it('Doesnt display delete button when bottom fixed is null', async function () {
            const {getByTestId,queryByTestId} = render(<AnimatedCard
                collapsableContent={<OrderCardContext.Provider value={orderCardContextMock}>
                    <AnimatedCardContext.Provider value={animatedCardContextMock}>
                        <ReducedOrder toFixed/>
                    </AnimatedCardContext.Provider>
        ...
        it('Changes height accordingly and arrow changes rotation', async function () {
            ...
            console.log(arrow.props.style[0].transform[0].rotateZ);
            //console.log(animatedView.props.children[0]);
